...
# Reproducibility switches (set BEFORE importing numpy/sklearn)
import os
os.environ["PYTHONHASHSEED"] = "0"
os.environ["OMP_NUM_THREADS"] = "1"  # helps reduce BLAS nondeterminism
...
output_dir = "C:/Users/vishn/OneDrive/Desktop/ML01/ABC-ANN_f"
...
# (Keep your original single split)
...
def generate_solution():
    return [
        ...
        random.randint(8, 128),   # h2 (your original had >=8; keeping as-is)
        random.randint(0, 128),   # h3 can be 0 (means 2-layer net)
...
for it in range(MAX_ITER):
    ...
    for i in range(NUM_BEES):
        ...
        while k == i:
            k = random.randint(0, NUM_BEES - 1)
        # keep-or-replace mix (your original move)
    ...
    # Onlooker phase with safe fitness normalization (use seeded rng for choice)
    ...
    for _ in range(NUM_BEES):
        i = int(rng.choice(NUM_BEES, p=probs))  # seeded selection
        phi = generate_solution()
        new_sol = [(solutions[i][j] if random.random() > 0.5 else phi[j]) for j in range(4)]
        new_fit = evaluate_solution(new_sol)
        if new_fit > fitness[i]:
...
with open(os.path.join(output_dir, "abc_ann_metrics.txt"), "w", encoding="utf-8") as f:
    f.write("ABC-ANN Model Evaluation (FoS)\n")
    f.write("=================================\n")
...
# Save predictions (optional; useful for external plotting)
...
print("\n✅ ABC-ANN (FoS) Training Completed")
...
plt.ylabel("Best $R^2$")
plt.title("ABC Fitness over Iterations (FoS)")
plt.grid(True)
save_both(os.path.join(hq_dir, "abc_fitness_plot"))

# Scatter helper (equal axes + embedded stats)
def scatter_ap(y_true, y_pred, title, fname, xlabel="Actual FoS", ylabel="Predicted FoS"):
    r2  = r2_score(y_true, y_pred)
    rmse = np.sqrt(mean_squared_error(y_true, y_pred))
...
# Line plots (clean layout)
def line_plot(y_true, y_pred, title, fname, ylabel="FoS"):
