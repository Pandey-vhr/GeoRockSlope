...
output_dir = "C:/Users/vishn/OneDrive/Desktop/ML01/GA-ANN_y"
...
# Inverse transforms and metrics (same pattern as ABC)
...
# Save fitness history & predictions for external use
...
plt.title("GA Fitness Progress (FoS)")
plt.grid(True)
save_png(os.path.join(hq_dir, "ga_fitness_progress.png"))

# Scatter helper: equal axes + embedded R2/RMSE
def scatter_actual_vs_pred(y_true, y_pred, title, fname, xlabel="Actual FoS", ylabel="Predicted FoS"):
    r2  = r2_score(y_true, y_pred)
...
line_plot(y_train_true, y_train_pred,
          "Actual vs Predicted (Train, FoS)",
          "ga_line_train.png")
line_plot(y_test_true,  y_test_pred,
          "Actual vs Predicted (Test, FoS)",
          "ga_line_test.png")

print("\n🎯 GA-ANN completed. Best config and model saved to:", output_dir)
